...
YOUR_USERNAME = os.environ.get("USER_NAME")
UPDATE_CHANNEL = "https://t.me/solitary_hacker"
...
# --- Partial Payment & Admin DB Helpers ---
...
def monitor_and_guide_error(
    ...
    if process.poll() is not None:
        try:
            ...
            match_js = re.search(
                r"(?:Cannot find module '(.+?)'|Error: Cannot find module '(.+?)')",
                log_content,
            )
            ...
            if match_py:
                ...
                missing_module = (match_js.group(1) or match_js.group(2)).split("/")[0].strip("'\"")
...
def run_js_script(
    ...
    try:
        ...
        # টার্মাক্স এবং স্ট্যান্ডার্ড লিনাক্স সার্ভারের জন্য node কমান্ডের সঠিক পাথ খুঁজে বের করা
        node_cmd = None
        possible_paths = [
...
def handle_callbacks(call):
    ...
    if data == "view_plans_cb":
        ...
        bot.send_message(
            ...
            "🚀 **এখন আপনার Python (.py), JS (.js) ফাইল মেসেজে পাঠান।**",
            parse_mode="Markdown",
        )
    ...
    # --- Interactive Module Installer Handler ---
        ...
        def do_install():
            ...
            res = subprocess.run(cmd, cwd=ufolder, capture_output=True, text=True)
            if res.returncode == 0:
                bot.edit_message_text(
    ...
    # --- Admin Callbacks ---
        ...
        msg = bot.send_message(
            call.message.chat.id,
            "📣 **সব ইউজারের কাছে পাঠানোর জন্য মেসেজটি লিখুন:**",
            parse_mode="Markdown",
        )
        bot.register_next_step_handler(msg, process_broadcast_message)

    elif data == "run_all_scripts" and user_id in admin_ids:
        bot.answer_callback_query(call.id, "🚀 Running all uploaded scripts...")
        count = 0
        for u_id, files in user_files.items():
            ufolder = get_user_folder(u_id)
            for fname, ftype in files:
                if not is_bot_running(u_id, fname):
                    fpath = os.path.join(ufolder, fname)
                    if ftype == "js":
                        threading.Thread(
                            target=run_js_script,
                            args=(fpath, u_id, ufolder, fname, call.message),
                        ).start()
                    else:
                        threading.Thread(
                            target=run_script,
        ...
        bot.send_message(
            call.message.chat.id,
            f"📊 **Bot Statistics:**\n\n👥 Active Users: `{len(active_users)}`\n👑 Total Admins: `{len(admin_ids)}`\n📁 Total Files Hosted: `{sum(len(f) for f in user_files.values())}`",
            parse_mode="Markdown",
        )
    ...
    # --- File Management Callbacks ---
        ...
        if os.path.exists(fpath):
            if not is_bot_running(u_id, fname):
                ...
                bot.answer_callback_query(call.id, "Script is already running!", show_alert=True)
        else:
...
def process_remove_subscription(message):
    try:
        sub_uid = int(message.text.strip())
        remove_subscription_db(sub_uid)
        bot.reply_to(message, f"✅ **Subscription removed for User ID:** `{sub_uid}`", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")
...
def process_add_admin(message):
    try:
        ...
        add_admin_db(new_adm)
        bot.reply_to(message, f"✅ **User `{new_adm}` is now an Admin!**", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")
...
def process_remove_admin(message):
    try:
        ...
        if rem_adm == OWNER_ID:
            ...
            return
        remove_admin_db(rem_adm)
        bot.reply_to(message, f"✅ **User `{rem_adm}` removed from Admins.**", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")
...
def process_broadcast_message(message):
    ...
    status_msg = bot.reply_to(message, "📢 **Broadcast পাঠানো শুরু হয়েছে...**", parse_mode="Markdown")
    
    for u_id in active_users:
        try:
            bot.send_message(u_id, text_to_broadcast, parse_mode="Markdown")
            success_count += 1
            time.sleep(0.1)
        except Exception:
            fail_count += 1
...
BUTTON_MAPPING = {
    ...
    "💻 𝗧𝗲𝗿𝗺𝗶𝗻𝗮𝗹 𝗖𝗺𝗱": lambda m: bot.reply_to(m, "💻 **Terminal Ready.** আপনার টার্মাক্স ব্যাকএন্ড স্ট্যাটাস স্বাভাবিক আছে।"),
